...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const body = JSON.parse(await request.text()) as { events?: unknown };
  ...
  for (const e of events) {
    ...
    if (e.kind === "pageview" || e.kind === "action") {
      inserts.push(
        sql`
          insert into l10.analytics_events (user_id, event_type, path, target)
          values (${user.id}, ${e.kind}, ${path}, ${str(e.target, 120)})
        `,
      );
